...
Object.keys(webpackConfig.entry).forEach(function (name) {
  ...
  webpackConfig.plugins.push(
    new HtmlWebpackPlugin({
      ...
      filename: utils.assetsPath('html/' + name + '.html'),
      // template: 'src/pages/layout/template.html',
      templateContent: function () {
        var template = fs.readFileSync(path.join(__dirname, '../src/pages/layout/template.html'),'utf-8');
        return template.replace(/<!--title-->/, Title[name] || '').replace(/<!--buildcss-->/, '<link rel="stylesheet" href="' + publicPath + '/static/common/common.min.css"/>').replace(/<!--buildjs-->/, '<script src=" ' + publicPath + '/static/common/lib.min.js"></script>');
      },
