...
var ENTER_TEMPLATE = require("./entry_template.js");
...
var funs = {
	...
	createRouter:function(){
		...
		function checkExis(path){
			return new Promise((resolve, reject) => {
				var entryPath = `./src/pages${path}.entry.js`;
				var vuePath   = `./src/pages${path}.vue`;
				...
				resolve({entryPath,vuePath, path});
			});
		}

		function confirm(pathsConfig){
			return new Promise((resolve, reject) => {
			...
			return new Promise((resolve, reject) => {
				...
				spawn('mkdir', ['-p',dir]).on('exit',function(){
					try{
						...
						fs.writeFileSync(vuePath,vueFileConten);
    ...
    spawn('node', ['build/dev-server.js'], {stdio: 'inherit'})
  },
	// 编译
	build:function(){
